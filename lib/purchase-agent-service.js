...
class PurchaseAgentService {
  constructor() {
    // Use environment variable for Railway deployment, fallback to localhost for dev
    this.apiUrl = process.env.PURCHASE_AGENT_URL || 'http://localhost:7777';
  ...
  async searchEquipmentImage(make, model) {
    const data = { make, model };

    try {
      const response = await axios.post(`${this.apiUrl}/api/images/equipment`, data);
      return response.data;
    } catch (error) {
      throw new Error(`Equipment image search failed: ${error.response?.data?.error || error.message}`);
    }
  }

  async searchPartImage(make, model, partName, oemNumber = null) {
    const data = { 
      make, 
      model, 
      part_name: partName,
    ...
    try {
      const response = await axios.post(`${this.apiUrl}/api/images/part`, data);
      return response.data;
    } catch (error) {
      throw new Error(`Part image search failed: ${error.response?.data?.error || error.message}`);
    }
  }
  ...
  async searchWeb(query, options = {}) {
    ...
    try {
      const response = await axios.post(`${this.apiUrl}/api/search/web`, data);
      return response.data;
    } catch (error) {
      throw new Error(`AI web search failed: ${error.response?.data?.error || error.message}`);
    }
  }

  async searchSerp(query, options = {}) {
    const data = {
      query,
      num_results: options.numResults || 10,
      location: options.location || null,
      domain: options.domain || 'google.com',
      language: options.language || 'en'
    };

    try {
      const response = await axios.post(`${this.apiUrl}/api/search/serp`, data);
      return response.data;
    } catch (error) {
      throw new Error(`SERP search failed: ${error.response?.data?.error || error.message}`);
    }
  }
}
