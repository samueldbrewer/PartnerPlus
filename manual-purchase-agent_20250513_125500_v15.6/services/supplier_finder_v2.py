...
def filter_raw_results(results, part_number):
    ...
    for result in results:
        ...
        commercial_indicators = ["price", "$", "buy", "shop", "order", "cart", "purchase", "add to cart", "in stock", "delivery", "shipping"]
        has_commercial = any(ind in combined_text for ind in commercial_indicators)
        
        # Part indicators  
        part_indicators = ["part", "replacement", "oem", "genuine", "component", "assembly", "caster", "brake", "swivel"]
        ...
        # Major supplier domains - always include these if they have part context
        major_suppliers = ["amazon.com", "ebay.com", "partstown.com", "webstaurantstore.com", 
                          "etundra.com", "zoro.com", "grainger.com", "mcmaster.com", "alibaba.com"]
        is_major_supplier = any(supplier in domain for supplier in major_suppliers)
        
        # Relaxed inclusion criteria - include if:
        # 1. Has part number in text OR
        # 2. Has commercial indicators OR  
        # 3. Is a major supplier with part context OR
        # 4. Is a product page
        should_include = (
            has_part_number or 
            has_commercial or 
            (is_major_supplier and has_part_context) or
            is_product_page
        )
        ...
        if should_include:
            filtered.append({
                ...
                "is_major_supplier": is_major_supplier
            })
...
def log_filtered_results(results):
    ...
    for i, result in enumerate(results[:10], 1):
        ...
        logger.info(f"   Part#: {result['has_part_number']}, Commercial: {result['has_commercial']}, Part Context: {result['has_part_context']}, Product Page: {result['is_product_page']}, Major Supplier: {result['is_major_supplier']}")
        logger.info("")
...
def rank_with_ai(results, part_number, make, model):
    ...
    for i, result in enumerate(results):
        results_for_ai.append({
            ...
            "is_major_supplier": result['is_major_supplier']
        })
...
2. **MAJOR SUPPLIERS**: Prioritize amazon.com, ebay.com, webstaurantstore.com, etundra.com, zoro.com, grainger.com
3. **PRODUCT DETAIL PAGES**: Strongly prefer direct product pages over category/listing pages
...
5. **Commercial indicators**: Price, buy, shop, order mentions, in stock status
6. **Deduplication**: Only include ONE result per domain (keep the best product page)
